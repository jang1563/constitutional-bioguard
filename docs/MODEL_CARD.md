...
![Size-peer Pareto: the 184M response head (crimson) is Pareto-dominated by Qwen3Guard-0.6B.](../results/figures/size_peer_pareto.png)

*Response-harm recall vs over-refusal (n=554), same items for all guards; bubble area ∝ parameters. Generated by `scripts/plot_size_peer_pareto.py`.*
