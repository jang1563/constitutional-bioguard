...
V8BD = ROOT / "models" / "deberta_bioguard_v8bd"
...
def main():
    ...
    rows_b = [json.loads(l) for l in open(ROOT / "data/external/fortress_safe_responses.jsonl") if l.strip()]
    ...
    json.dump(out, open(ROOT / "results" / "v8bd_compare.json", "w"))
    print("\nsaved -> results/v8bd_compare.json")
