#!/usr/bin/env python
"""
Phase 3B Ablation — constitutional_bioguard v8b (query-only, no response)

Pre-response filter variant: classifier sees ONLY the query, no response.
Comparison with v8 (query+response) reveals response leakage.
...
Usage:
    python scripts/experiments/train_v8_baseline.py [--unsafe-weight 1.5] [--epochs 5]
"""
    ...
    for name in ("train", "val", "ood_fpr", "ood_fnr"):
        ...
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                # Query-only: no response (pre-response filter ablation)
                text = query.strip()
                records.append({
                    "text": text,
                    "label": label,
...
def main():
    ...
    parser.add_argument("--output-dir", type=str,
                        default=str(MODELS / "deberta_bioguard_v8b_query_only"))
    ...
    report_path = RESULTS / "phase3b_query_only_eval.json"
