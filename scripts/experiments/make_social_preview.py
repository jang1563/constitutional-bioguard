...
1280x640 (GitHub's recommended 2:1). The card leads with the contribution
(the evaluation harness + its sample-size finding), with the dual-mode
classifier and the owned negative result framed as what the harness red-teamed.

Run: python scripts/experiments/make_social_preview.py
...
def main():
    ...
    # tagline
    ax.text(0.07, 0.585,
            "A biosafety guard-model evaluation harness —\nand the dual-mode classifier it red-teamed.",
            color=MUTE, fontsize=20, linespacing=1.25, va="top")

    # three bullets with colored dots: build, finding, owned-negative
    bullets = [
        (GREEN, "Dual-mode guard: response + prompt heads (2x184M DeBERTa-v3),"
                " constitution-driven data."),
        (AMBER, "Finding: guard leaderboards flip rank from n=30 to n=500 —"
                " small-n \"best recall\" is underpowered."),
        (CRIMSON, "5 self-audits, each reversed a prior claim —"
                  " including ranking my own model down."),
    ]
    y = 0.40
